...
В деве этот файл подключается дважды: ссылкой `?direct` — чтобы браузер получил css вместе с разметкой и страница не моргнула нестилизованной, — и модулем, ради горячей замены. Цена: в деве файл едет по сети два раза. Стили самих роутов в деве по-прежнему приходят импортом из клиентского модуля, то есть после разметки: какие именно css тянет роут, без сборки знает только vite.
